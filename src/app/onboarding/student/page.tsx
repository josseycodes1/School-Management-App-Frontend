...
export default function StudentOnboarding() {
  ...
  const validateField = (name: string, value: any): string | undefined => {
    switch (name) {
      ...
      case 'birth_date':
        ...
        // Student-specific age validation (5-25 years)
        const minStudentAge = new Date();
        minStudentAge.setFullYear(today.getFullYear() - 25);
        const maxStudentAge = new Date();
        maxStudentAge.setFullYear(today.getFullYear() - 5);
        if (birthDate < minStudentAge || birthDate > maxStudentAge) {
          return 'Student age should be between 5 and 25 years';
        }
        return undefined;

      case 'parent_name':
        if (!value) return 'Parent/Guardian name is required';
        if (value.length < 2) return 'Please enter a valid name';
        return undefined;

      case 'parent_contact':
        if (!value) return 'Parent/Guardian contact is required';
        const cleanedParentPhone = value.replace(/[\s\-\(\)\+]/g, '');
        const parentPhoneRegex = /^[0-9]{8,15}$/;
        if (!parentPhoneRegex.test(cleanedParentPhone)) {
  ...
  const validateForm = (): boolean => {
    ...
    const requiredFields: (keyof FormData)[] = [
      ...
      'parent_name', 'parent_contact', 'class_level'
    ];
    ...
    const checkOnboardingStatus = async () => {
      try {
        ...
        setFormData(prev => ({
          ...
          admission_number: existingData.admission_number || ''
        }));
  ...
  const calculateProgress = (): void => {
    ...
    const requiredFieldsStatus = {
      ...
      photo: hasPhoto,
      admission_number: true // Auto-generated, always considered complete
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const allFields = ['phone', 'address', 'gender', 'birth_date', 'parent_name', 'parent_contact', 'class_level', 'photo'];
