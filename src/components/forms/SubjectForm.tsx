...
const schema = z.object({
  ...
  assigned_class: z.string().min(1, "Class assignment is required"),
});
  ...
  useEffect(() => {
    if (data) {
      reset({
        ...
        assigned_class: data.assigned_class?.toString() || "",
      });
    }
  }, [data, reset]);
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      const payload = {
        ...
        assigned_class: parseInt(formData.assigned_class),
      };
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl p-6">
          <div className="flex justify-center items-center h-32">
            <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-josseypink1"></div>
          </div>
        </div>
      </div>
    );
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="bg-josseypink1 text-white p-4 rounded-t-lg">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold">
              {type === "create" ? "Add New Subject" : "Edit Subject Details"}
            </h2>
            <button 
              onClick={onClose}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="w-full">
              <label className="block mb-2 text-sm font-medium text-gray-900">Assigned Class*</label>
              <select
