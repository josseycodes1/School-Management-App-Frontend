...
const ExamListPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchExams = async () => {
      try {
        ...
        if (!backendUrl) throw new Error("NEXT_PUBLIC_BACKEND_URL is undefined");

        console.log("Fetching exams from:", `${backendUrl}/api/assessment/exams/`);

        const res = await axios.get(`${backendUrl}/api/assessment/exams/`, {
        ...
        setError("Failed to load exams. Make sure your backend is running and the token is valid.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      await axios.delete(`${backendUrl}/api/assessment/exams/${id}/`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
                      ...
                      {(role === "admin" || role === "teacher") && (
                        <>
                          <FormModal
                            ...
                            type="update"
                            data={exam}
                            onSuccess={(updatedExam) => setExams(exams.map(e => e.id === updatedExam.id ? updatedExam : e))}
                            trigger={
                          ...
                          />
                          <button onClick={() => setDeleteConfirm(exam.id)} className="text-white hover:text-pink-100 bg-josseypink1 hover:bg-josseypink2 p-1 rounded">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button onClick={() => handleDelete(deleteConfirm)} className="px-4 py-2 bg-josseypink1 text-white rounded-md hover:bg-josseypink2">Delete</button>
            </div>
          </div>
        </div>
      )}
